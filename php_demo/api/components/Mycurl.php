...
file_put_contents(DEBUG_CURL_LOG, Myfun::generateLogTime().' my_vposttuniu => url => '. $url ."\n", FILE_APPEND);
...
file_put_contents(DEBUG_CURL_LOG, Myfun::generateLogTime().' my_vpostjson => data => '. $url."\n".$data ."\n", FILE_APPEND);
	...
	// post请求
	public static function my_vpostjsonNogizp($url, $data='', $cookie_name='', $referer='', $proxy_enable = PROXY_ENABLE){ // 模拟提交数据函数
		$data = empty($data) ? '' : json_encode($data);
file_put_contents(DEBUG_CURL_LOG, Myfun::generateLogTime().' my_vpostjson => data => '. $url."\n".$data ."\n", FILE_APPEND);
		$curl = curl_init(); // 启动一个CURL会话
		curl_setopt($curl, CURLOPT_URL, $url); // 要访问的地址
	...
	// post请求
	public static function my_vpostjson_gzip($url, $data='', $cookie_name='', $referer='', $proxy_enable = PROXY_ENABLE){ // 模拟提交数据函数
		$data = empty($data) ? '' : json_encode($data);
file_put_contents(DEBUG_CURL_LOG, Myfun::generateLogTime().' my_vpostjson => data => '. $url."\n".$data ."\n", FILE_APPEND);
		$curl = curl_init(); // 启动一个CURL会话
		curl_setopt($curl, CURLOPT_URL, $url); // 要访问的地址
...
file_put_contents(DEBUG_CURL_LOG, Myfun::generateLogTime().' my_vgetbus => param => '. $param ."\n", FILE_APPEND);
    ...
    // get请求
    public static function my_vget_header($url, $data='', $cookie_name = '', $referer=CURL_REFERER,$header=0, $proxy_enable = PROXY_ENABLE){ // 模拟提交数据函数
        $data = empty($data) ? '' : http_build_query($data);
        //var_dump($data);
        $curl = curl_init(); // 启动一个CURL会话
        curl_setopt($curl, CURLOPT_URL, $url.'?'.$data); // 要访问的地址
	...
	// 腾讯专用post请求
	public static function my_txpost($url, $data='', $IMEI='', $APN=''){ // 模拟提交数据函数
		$data = empty($data) ? '' : http_build_query($data);
//file_put_contents(DEBUG_CURL_LOG, Myfun::generateLogTime().' my_vpostjson => data => '. $url."\n".$data ."\n", FILE_APPEND);
		$curl = curl_init(); // 启动一个CURL会话
		curl_setopt($curl, CURLOPT_URL, $url); // 要访问的地址
		...
		curl_setopt($curl, CURLOPT_USERAGENT, CURL_USER_AGENT); // 模拟用户使用的浏览器
		//curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1); // 使用自动跳转
		curl_setopt($curl, CURLOPT_REFERER, TX_NEWS_REFERER); // 自动设置Referer
		curl_setopt($curl, CURLOPT_POST, 1); // 发送一个常规的Post请求
		curl_setopt($curl, CURLOPT_POSTFIELDS, $data); // Post提交的数据包
		curl_setopt($curl, CURLOPT_TIMEOUT, CURL_TIMEOUT); // 设置超时限制防止死循环
		curl_setopt($curl, CURLOPT_HEADER, 0); // 显示返回的Header区域内容
		...
		$temp = rand()%10000000;
		$IMEI = empty($IMEI) ? $temp : $IMEI;
		$APN = empty($APN) ? TX_NEWS_APN : $APN;
		curl_setopt($curl, CURLOPT_HTTPHEADER, array(
			'V-IMEI: '.$IMEI,
			'Apn: '.$APN,
			'Content-Type: '.TX_NEWS_CT,
			'Accept: */*',
