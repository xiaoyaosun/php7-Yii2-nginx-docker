...
mkdir -p /opt/logs/normal
mkdir -p /opt/logs/debug
mkdir -p /opt/app/wk
mkdir -p /opt/app/runtime
mkdir -p /opt/app/cookie_path
mkdir -p /opt/app/app-assets
mkdir -p /opt/app/staticfile
chown -R www:www /opt
...
# add php.ini configure
[ -f "${Php_Etc_Dir}/php.ini" ] || mkdir -p $Php_Etc_Dir
cat >> ${Php_Etc_Dir}/php.ini << EOF
